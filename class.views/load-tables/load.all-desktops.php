...
if (isset($_POST["load"]) || isset($_POST["search"])) {
  ...
  if(isset($_POST["load"])){ //if a post variable called load was sent then the query will be the following...
    ...
    $sql = "SELECT Device_id,  Device_name, Device_AssetTag_id, User_id FROM devices WHERE Device_Type_id = 3  && Status = 1 ORDER BY Device_AssetTag_id LIMIT 15;";
    ...
    $desktopID = $_POST["search"];

    //search query
    $sql = "SELECT Device_id,  Device_name, Device_AssetTag_id, User_id FROM devices WHERE Device_Type_id = 3 && Device_AssetTag_id LIKE '$desktopID%' && Status = 1  LIMIT 10;";
  }
  ...
  if (mysqli_num_rows($result) > 0 ){ //there are laptops to show
      while ($row = mysqli_fetch_array($result)) { ?>
        <!-- calling javascript function called fill_searchDesktops in class.load-tables.js by passing the fetched result as a parameter-->
        <div hidden onclick='fill_searchDesktops("<?php echo $row['Device_AssetTag_id']; ?>")'></div>
        <?php
        $user = $row["User_id"];
