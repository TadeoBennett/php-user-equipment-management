...
if (isset($_POST["load"]) || isset($_POST["search"])) {
  ...
  if(isset($_POST["load"])){ //if a post variable called load was sent then the query will be the following...
    ...
    $sql = "SELECT Device_id, Device_name, Device_AssetTag_id, User_id FROM devices WHERE Device_Type_id = 8 && Status = 1  ORDER BY Device_AssetTag_id LIMIT 15;";
    ...
    $printerID = $_POST["search"];

    //search query
    $sql = "SELECT Device_id, Device_name, Device_AssetTag_id, User_id FROM devices WHERE Device_Type_id = 8 && Device_AssetTag_id LIKE '$printerID%' && Status = 1 LIMIT 10;";
  }
  ...
  if (mysqli_num_rows($result) > 0){ //there are printers to show
      while ($row = mysqli_fetch_array($result)) { ?>
        <!-- calling javascript function called fill_searchPrinters in class.load-tables.js by passing the fetched result as a parameter-->
        <div hidden onclick='fill_searchPrinters("<?php echo $row['Device_AssetTag_id']; ?>")'></div>
        <?php
        $user = $row["User_id"];
